...
export const Bookings = () => {
  ...
  const filteredBookings = bookings.filter(
    (booking: {
      ...
      const userNameMatch = booking.userId?.fullName
        ? booking.userId.fullName.toLowerCase().includes(searchLower)
        : false;
      const hostelNameMatch = booking.hostelId?.hostel_name
        ? booking.hostelId.hostel_name.toLowerCase().includes(searchLower)
        : false;
      const providerNameMatch = booking.providerId?.fullName
        ? booking.providerId.fullName.toLowerCase().includes(searchLower)
        : false;
      const bookingIdMatch = booking._id
        ? booking._id.toLowerCase().includes(searchLower)
        : false;

      const matchesSearch =
        userNameMatch || hostelNameMatch || providerNameMatch || bookingIdMatch;
  ...
  const getStatusClass = (status: string) => {
    switch (status) {
      case "completed":
        return "bg-green-900 text-green-300 border border-green-800";
      case "pending":
        return "bg-[#3d3f45] text-[#C8ED4F] border border-[#4a4c52]";
      case "cancelled":
  ...
  return (
    <div className="p-6 mt-4 max-w-7xl mx-auto bg-[#242529] rounded-lg shadow-md h-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <h2 className="text-2xl font-dm_sans font-semibold text-[#C8ED4F]">
          Booking Management
        </h2>
      ...
      {filteredBookings.length === 0 ? (
        ...
        <div className="space-y-4">
          {currentBookings.map((booking: Booking) => (
            <div
              key={booking._id}
              className="bg-[#2A2B2F] rounded-lg shadow-sm overflow-hidden border border-[#3d3f45] hover:shadow-md transition-all duration-300"
            >
                ...
                <div className="grid grid-cols-12 gap-2 items-center p-4">
                  ...
                  <div className="col-span-3 flex items-center">
                    ...
                    <div>
                      <h3 className="font-semibold text-base truncate text-white">
                        {booking.hostelId?.hostel_name || "Unknown Hostel"}
                      </h3>
                  ...
                  <div className="col-span-2 flex items-center">
                    ...
                    <div
                      ...
                      title={booking.userId?.fullName || "Unknown User"}
                    >
                  ...
                  <div className="col-span-2 flex items-center">
                    ...
                    <div
                      className="text-sm truncate text-gray-300"
                      title={booking.providerId?.fullName || "Unknown Provider"}
                    >
                ...
                <div className="px-4 py-2 bg-[#2F3033] border-b border-[#3d3f45]">
                  <div className="flex items-center text-gray-300">
                    ...
                    <div
                      ...
                      title={getHostelAddress(booking)}
                    >
                      {getHostelAddress(booking)}
                    </div>
                  </div>
                    ...
                    <div className="bg-[#323338] p-4 rounded-lg shadow-sm border border-[#3d3f45]">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-start">
                          ...
                          <span className="text-gray-200 text-sm font-medium">
                            {booking.userId?.phone || "No phone available"}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#323338] p-4 rounded-lg shadow-sm border border-[#3d3f45]">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-start">
                          ...
                          <span className="text-gray-200 text-sm font-medium">
                            {booking.providerId?.email || "No email available"}
                          </span>
                        </div>
                        <div className="flex items-start">
                          <span className="text-gray-400 w-20 text-sm">
                          ...
                          <span className="text-gray-200 text-sm font-medium">
                            {booking.providerId?.phone || "No phone available"}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#323338] p-4 rounded-lg shadow-sm border border-[#3d3f45] md:col-span-3">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                        ...
                        <div className="flex flex-col items-start">
                          ...
                          <span
                            className={`text-sm font-medium px-2 py-0.5 rounded-md mt-1 ${getStatusClass(
                              booking.paymentStatus
                            )}`}
                          >
                            {booking.paymentStatus.charAt(0).toUpperCase() +
                      ...
                      booking.selectedFacilities.length > 0 ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                          {booking.selectedFacilities.map((facility, index) => (
                            <div
                              key={index}
                              className="bg-[#3d3f45] p-3 rounded-md border border-[#4a4c52]"
                            >
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <span className="text-[#C8ED4F] text-sm font-medium">
                                  ₹{facility.totalCost}
                                </span>
                              </div>
